...
class KFormTest extends Component {
  onLogin = () => {
    // 校验
    const { validateFields, getFieldsValue ,getFieldValue} = this.props
    validateFields((isValid, data) => {
      if (isValid) {
        const data1 = getFieldsValue()
        const curData = getFieldValue('username')
        console.log(data1,'getFieldsValue')
        console.log(curData,'getFieldValue')
        console.log('登录！！！！')
      } else {
        alert('校验失败')
      }
